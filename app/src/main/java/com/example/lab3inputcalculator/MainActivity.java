...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean isEqualPressed = false;    // Track when equals is pressed
    ...
    public void onClickDecimal(View view) {
        if (!hasDecimal) {  // Only allow one decimal point
            ...
            if (display.getText().toString().isEmpty()) {
                ...
                display.append(".");  // Append the decimal to the existing number
            }

            hasDecimal = true;  // Set the flag to indicate a decimal has been added
        }
    }
    ...
    public void onClickEquals(View view) {
        calculate();
        isEqualPressed = true;  // Set the flag to indicate equals was pressed
    ...
    private void calculate() {
        ...
        if (input.isEmpty() || !input.matches("[-+]?\\d*\\.?\\d+")) {
            display.setText("Error");  // Display error if input is invalid
            valueOne = 0;  // Set valueOne to 0 as a fallback
            return;
        }
